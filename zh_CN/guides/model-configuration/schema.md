...
* 供应商规则基于 [Provider](schema.md#Provider) 实体。
* 模型规则基于 [AIModelEntity](schema.md#AIModelEntity) 实体。
...
* `help` (object) \[optional] 帮助信息
  ...
  * `url` (object) 帮助链接，i18n
    ...
    * `en_US` (string) 英文链接
* `supported_model_types` (array\[[ModelType](schema.md#ModelType)]) 支持的模型类型
* `configurate_methods` (array\[[ConfigurateMethod](schema.md#ConfigurateMethod)]) 配置方式
* `provider_credential_schema` ([ProviderCredentialSchema](schema.md#ProviderCredentialSchema)) 供应商凭据规格
* `model_credential_schema` ([ModelCredentialSchema](schema.md#ModelCredentialSchema)) 模型凭据规格
...
* `label` (object) \[optional] 模型展示名称，i18n，可设置 `en_US` 英文、`zh_Hans` 中文两种语言
  ...
  * `en_US` (string) 英文标签名
* `model_type` ([ModelType](schema.md#ModelType)) 模型类型
* `features` (array\[[ModelFeature](schema.md#ModelFeature)]) \[optional] 支持功能列表
* `model_properties` (object) 模型属性
  * `mode` ([LLMMode](schema.md#LLMMode)) 模式 (模型类型 `llm` 可用)
...
* `parameter_rules` (array\[[ParameterRule](schema.md#ParameterRule)]) \[optional] 模型调用参数规则
* `pricing` ([PriceConfig](schema.md#PriceConfig)) \[optional] 价格信息
...
* `credential_form_schemas` (array\[[CredentialFormSchema](schema.md#CredentialFormSchema)]) 凭据表单规范
...
* `label` (object) 表单项标签名
  * `en_US`(string) 英文
  * `zh_Hans` (string) \[optional] 中文
* `type` ([FormType](schema.md#FormType)) 表单项类型
* `required` (bool) 是否必填
* `default`(string) 默认值
* `options` (array\[[FormOption](schema.md#FormOption)]) 表单项为 `select` 或 `radio` 专有属性，定义下拉内容
* `placeholder`(object) 表单项为 `text-input` 专有属性，表单项 PlaceHolder
  * `en_US`(string) 英文
  * `zh_Hans` (string) \[optional] 中文
* `max_length` (int) 表单项为`text-input`专有属性，定义输入最大长度，0 为不限制。
* `show_on` (array\[[FormShowOnObject](schema.md#FormShowOnObject)]) 当其他表单项值符合条件时显示，为空则始终显示。
